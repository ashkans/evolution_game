...
from my_helpers import link_descendants
...
class Bobject(object):
    ...
    def sub_object_move_to(
        ...
        if child_ids is None:
            child_ids = []
        elif isinstance(child_ids, int) or isinstance(child_ids, float):
            child_ids = [child_ids]
        ...
        if new_location is not None:
            ...
            for cid in child_ids:
                obj.children[cid].location = new_location
            obj.keyframe_insert(data_path="location", frame=end_frame)
        if new_scale is not None:
            ...
            if isinstance(new_scale, int) or isinstance(new_scale, float):
                new_scale = [new_scale] * 3
            for cid in child_ids:
                obj.children[cid].scale = new_scale
            obj.keyframe_insert(data_path="scale", frame=end_frame)
        if new_angle is not None:
            obj.keyframe_insert(data_path="rotation_euler", frame=start_frame - 1)
            for cid in child_ids:
                obj.children[cid].rotation_euler = new_angle
            obj.keyframe_insert(
                data_path="rotation_euler",
                frame=end_frame
            )
